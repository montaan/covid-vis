...
import { COVIDTree as UnitedStatesCOVIDTree } from "../../data/UnitedStates";
...
const fullscreenSupported =
  document.exitFullscreen && !isInWebAppChrome && !isInWebAppiOS;
...
class MainApp extends React.Component<MainAppProps, MainAppState> {
  ...
  constructor(props: MainAppProps) {
    ...
    setTimeout(() => {
		...
		// this.mountFSEntry("/World", "United States", UnitedStatesCOVIDTree);
		// this.mountFSEntry("/", "Europe", NUTSTree);
		// const Europe = getOrCreateDir(this.state.fileTree.tree.entries.get("World"), "Europe");
		// this.mountFSEntry("/World/Europe", "Portugal", PortugalCOVIDTree);
		// this.mountFSEntry("/World/Europe", "Sweden", SwedenCOVIDTree);
		// this.mountFSEntry("/World/Europe", "Finland", FinlandCOVIDTree);
    }, 100);
  }
  ...
  processThumbnailRequest = async ({
    tree,
    thumbnails
  }: LoadThumbnailsWorkItem) => {
    let thumbResults;
  ...
  setNavigationTarget = (navigationTarget: string) => {
    if (this.state.navigationTarget !== navigationTarget)
  ...
  filterCommits(repo: string, commitFilter: CommitFilter): Commit[] {
    ...
    if (date)
      [year, month, day] = date.split("-").map((v: string) => parseInt(v));
    ...
    for (var i = 0; i < il; ++i) {
      ...
      const authorSearchHit =
        !authorSearch || c.author.toLowerCase().includes(authorSearch);
      var dateHit = !date;
      if (year !== undefined) {
      ...
      if (pathHit && authorHit && searchHit && authorSearchHit && dateHit)
        commits.push(c);
    }
    return commits;
  }
  ...
  getHitType(rawQueryRE: RegExp, filename: string, snippet?: string) {
    if (/(^|\/)doc(s|umentation)?\//i.test(filename))
      return HitType.DOCUMENTATION;
  ...
  addLinks = (links: TreeLink[]) =>
    this.setLinks(this.state.links.concat(links));

  setDependencies = (dependencies: TreeLink[]) =>
    this.setState({ dependencies });

  setCommitsVisible = (commitsVisible: boolean) =>
    this.setState({ commitsVisible });
  ...
  renameRepo = async (repo: RepoInfo, newName: string): Promise<void> => {
    const res = await this.props.api.post("/repo/rename", {
      name: repo.name,
      newName
    });
    if (res === "OK") {
  ...
  getPathFilesystems() {
    return getAllFilesystemsForPath(
      this.state.fileTree.tree,
      this.state.navigationTarget
    );
  }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <div id="debug" />
        {fullscreenSupported && (
          <div id="fullscreen" onClick={this.fullscreenOnClick} />
        )}

        {this.state.repoError && (
          <div id="repoError">{this.state.repoError}</div>
        )}
